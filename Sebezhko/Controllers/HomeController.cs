...
namespace Sebezhko.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult Card(int id)
        {
            ...
            return View(product); // Возвращаем представление с продуктом
        }
        ...
        public IActionResult AddReview(int productId, string comment, int rating)
        {
            ...
            {
                ModelState.AddModelError("", "Рейтинг должен быть от 1 до 5.");
                return RedirectToAction("Card", new { id = productId });
            }
            ...
            if (string.IsNullOrEmpty(userIdString))
            {
                ModelState.AddModelError("", "Вы должны быть авторизованы для оставления отзыва.");
                return RedirectToAction("SignIn");
            }
            ...
            return RedirectToAction("Card", new { id = productId }); // Перенаправляем обратно на страницу товара
        }
